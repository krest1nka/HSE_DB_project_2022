...
CREATE TABLE project.paintings(
    ...
    painting_creation_year date check ( painting_creation_year <= current_date), -- year???
...
select painting_name from project.paintings where artist_id = 3;

select sum(storage_capacity) from project.storages;

delete from project.owners where owner_id = 3;

insert into project.owners(owner_name, owner_surname)
values ('Kristina', 'Trofimova');

delete from project.owners where owner_name = 'Kristina';
...
select city_name, sum(storage_capacity) as total_capacity from project.storages
...
select owner_name || ' ' || owners.owner_surname as owner, sum(deal_price) as total_sales from project.owners
...
-- Для каждой картины вывести актуального владельца.
...
with extended_ownership_history as
    (select painting_name,
            ...
            valid_from,
            valid_until,
            rank() over (partition by painting_name order by valid_from desc) as rank
...
select painting_name, owner from extended_ownership_history
where rank = 1;

-- Вывести количество картин у каждого владельца в период с по

-- Что-то еще. Запросы придумывать сложно
...
create view project_views.owners as
    (select owner_name, overlay(owner_surname placing '****' from 2) from project.owners);

drop view if exists project_views.artists;
create view project_views.artists as
    (select artist_name as name, artist_surname as surname from project.artists);

drop view if exists project_views.paintings;
create view project_views.paintings as
    (select artist_id, painting_name, painting_creation_year from project.paintings);

drop view if exists project_views.cities;
create view project_views.cities as
    (select city_name as city, country_name as country from project.cities);

drop view if exists project_views.storages;
create view project_views.storages as
    (select city_id, storage_name, storage_capacity from project.storages);

-- ownership_history, owners_x_deals, deals
